"""
Build a self-contained Google Colab notebook for the MIT-BIH experiments.

Embeds the MIT-BIH dataset (base64-encoded zip) and all source code
...
def strip_local_imports(content):
    local_modules = [
        'dataset', 'dataset_mitbih', 'models', 'models_mitbih',
...
cells.append(create_cell("markdown",
    "# Early-Exit Neural Network Architectures — MIT-BIH\n"
    ...
    "**MIT-BIH Arrhythmia** dataset (5-class, 187-length heartbeats, 125 Hz)."
))

# ─── Setup: embed data ───
cells.append(create_cell("markdown",
    "## 0. Setup & Data\n"
    "The MIT-BIH dataset is embedded directly in this notebook (base64-encoded zip).\n"
    "Just run this cell — no Google Drive mount needed."
))

print("Packaging MIT-BIH data into zip...")
shutil.make_archive('mitbih_data', 'zip', 'data/mitbih')
with open('mitbih_data.zip', 'rb') as f:
...
print("Extracting embedded MIT-BIH dataset...")
os.makedirs('data', exist_ok=True)
with open('data/mitbih_data.zip', 'wb') as f:
    f.write(base64.b64decode("{encoded_data}"))

with zipfile.ZipFile('data/mitbih_data.zip', 'r') as zip_ref:
    zip_ref.extractall('data/mitbih')

# 2. Set environment paths
DATA_DIR = './data/mitbih'
PLOTS_DIR = './plots_mitbih'
RESULTS_DIR = './results_mitbih'

os.makedirs(PLOTS_DIR, exist_ok=True)
os.makedirs(RESULTS_DIR, exist_ok=True)
print("Setup complete! Data is ready in ./data/mitbih")
...
# ─── Source code cells ───
# We include both the original shared modules AND the MIT-BIH-specific ones.
# The MIT-BIH models use the shared building blocks (PolicyHead, ClassifierHead, etc.)
# from models.py, so we must include models.py first.

files_to_add = [
    ("1. MIT-BIH Dataset & Preprocessing", "dataset_mitbih.py"),
    ("2. Base Models (Architectures & Losses)", "models.py"),
    ("3. MIT-BIH Models (Short-Sequence Architectures)", "models_mitbih.py"),
...
cfg = MITBIHExperimentConfig(
    ...
    warmup_epochs=10,    # Full epochs — Colab GPU/TPU is fast enough
    joint_epochs=10,
)
...
print("\\n[DONE] MIT-BIH ablation complete.")
"""
...
# pipeline.generate_visualizations()
# pipeline.save_results()
# print("\\n[DONE] All additional MIT-BIH experiments complete.")
"""
