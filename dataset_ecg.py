...
def ensure_ecg_data_exists(data_path):
...
class ECGDataset(Dataset):
    def __init__(self, data_path, split="train", use_freq_bands=True):
        ...
        self.y = np.load(os.path.join(data_path, f"y_{split}.npy")).astype(np.int64)
...
def get_ecg_dataloaders(data_dir, batch_size=64, use_freq_bands=True):
    ensure_ecg_data_exists(data_dir)

    train_ds = ECGDataset(data_dir, "train", use_freq_bands=use_freq_bands)
    val_ds = ECGDataset(data_dir, "val", use_freq_bands=use_freq_bands)
    test_ds = ECGDataset(data_dir, "test", use_freq_bands=use_freq_bands)

    labels = train_ds.y
    class_counts = np.maximum(np.bincount(labels, minlength=5), 1)
