"""
main_ecg.py — Early Exit Architecture Research Pipeline for ECG Dataset.

Mirrors the Bonn pipeline (main.py) but configured for the ECG
...
from dataset_ecg import get_ecg_dataloaders
...
# ──────────────────────────────────────────────
# Configuration — ECG specific
# ──────────────────────────────────────────────

@dataclass
class ECGExperimentConfig:
    """Centralized configuration for ECG experiments."""
    ...
    seq_len: int = 187               # ECG heartbeat length
...
# ──────────────────────────────────────────────
# Model Registry (same architectures, adapted for ECG)
# ──────────────────────────────────────────────
...
# ──────────────────────────────────────────────
# Research Pipeline (ECG)
# ──────────────────────────────────────────────

class ECGResearchPipeline:
    """Pipeline for ECG Early-Exit experiments, mirroring the Bonn pipeline."""

    def __init__(self, config: ECGExperimentConfig):
        self.config = config
        self.device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        print(f"[ECG Pipeline] Device: {self.device}")
        print(f"[ECG Pipeline] Dataset: ECG Arrhythmia")
        print(f"[ECG Pipeline] Classes: {config.num_classes}, Seq Len: {config.seq_len}")

        os.makedirs(config.plots_dir, exist_ok=True)
        os.makedirs(config.results_dir, exist_ok=True)

        self.train_dl, self.val_dl, self.test_dl, self.class_weights = get_ecg_dataloaders(
    ...
    def _build_model(self, config_dict):
        """Factory method to construct the appropriate network (small blocks for ECG)."""
    ...
    def run_architecture_ablation(self, energy_lambdas=None):
        ...
        for e_lambda in energy_lambdas:
            print(f"\n{'='*60}")
            print(f"[ECG] Energy Lambda = {e_lambda}")
            print(f"{'='*60}")
    ...
    def run_model_size_experiment(self):
        ...
        print(f"\n{'='*60}")
        print("[ECG] Model Size Scaling Experiment")
        print(f"{'='*60}")
    ...
    def run_pruning_experiment(self, prune_ratios=None):
        ...
        pruning_results = []
        print(f"\n{'='*60}")
        print("[ECG] Structured Pruning Experiment")
        print(f"{'='*60}")
    ...
    def run_threshold_strategy_comparison(self):
        ...
        print(f"\n{'='*60}")
        print("[ECG] Threshold Strategy Comparison")
        print(f"{'='*60}")
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Early Exit Architecture Research — ECG Dataset")
    ...
    cfg = ECGExperimentConfig(
        base_seed=args.seed,
        num_trials=args.trials,
        threshold_strategy=args.strategy,
    )
    pipeline = ECGResearchPipeline(cfg)
    ...
    if args.run_all or args.run_tuning:
        print("\n" + "="*60)
        print("HYPERPARAMETER SEARCH — ECG")
        print("="*60)
    ...
    print("\n[DONE] All ECG experiments complete.")
