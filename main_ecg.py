...
from models import (GenericEarlyExitNetSmall, AdaptiveEarlyExitNetSmall,
...
class Pipeline:
    ...
    def _build(self, mc):
        if mc.get("adaptive"):
            return AdaptiveEarlyExitNetSmall(self.cfg.in_channels, mc["channels"],
                                             self.cfg.num_classes, self.cfg.seq_len).to(self.device)
        return GenericEarlyExitNetSmall(self.cfg.in_channels, mc["channels"],
                                        self.cfg.num_classes, self.cfg.seq_len).to(self.device)
    ...
    def run_strategies(self):
        strategy_results = {}
        mc = {"channels": [64, 64, 64], "is_baseline": False, "adaptive": False}
        set_seed(self.cfg.base_seed); model = self._build(mc); self._train(model, mc, self.cfg.base_seed)
    ...
    def save(self):
        ...
        out = {"dataset": "ECG Arrhythmia", "config": vars(self.cfg), "results_summary": self.results,
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Early Exit Research — ECG")
    ...
    if not run_any: args.run_ablation = True
